...
\subsubsection{Data}\label{Methodology:data}
...
\subsection{Data Collection //TODO}
...
Using the set of annotated scans we ran an algorithm to combine the three separate layers into a single image as depicted in Fig. \ref{fig:merged-layers}

\begin{figure}[H]
    \label{fig:merged-layers}
    \centering
...
\begin{figure}[H]
    ...
    \caption{Segmentation Map used for training (Maybe we can put the names of the layers in each one of the compartments)}
\end{figure}
...
\begin{lstlisting}
for directory_path in glob.glob("data/images/A"):
...
We use LabelEncoder \cite{scikit-learn} to encode our label with a value between 0 and n-classes-1, where n is the number of distinct label, here we have four: the vitreous, retina, choroid and sclera. This will allow us to add weight to each of our class and help to reduce class in-balance in our model. For instance the vitreous is very easy to identify for our model, we therefore want it to spend less time on it that the choroid or the retina. To do so the class 0 (vitreous) has a weight of 0.2 and the retina 10, meaning that the model will spend 5 times more time on the class 1 compared to the 0 and so on.
...
Then we need to encode our label from categorical features to their numeric representation, as it is required for neural network. We use to\_categorical from Keras \cite{chollet2015keras} to convert our current class vector to its binary matrix representation.
...
Usually deep learning requires a large volume of data to have good performances, but as we saw in sec. \ref{Methodology:data} we only have 755 OCT B-scans. We already use U-net (sec. \ref{tech:unet}) to overcome this problem because it was develop specifically for small dataset usage, one top of that data augmentation can be added to artificially increase the number of data we feed to the model. Data augmentation is a widely adopted approach for increasing the amount of training data \cite{AugAndEval:2017} but the quality of the augmented data may not always be precise as too harsh modifications can lead to significant loss in the data. This is even more true in our case because the OCT B-scans are extremely precise (nanometer)(see sec. \ref{Methodology:data}), we therefore want to be cautious when choosing the augmentation parameters.
We chose a rotation of maximum 5 degrees because after some testing we realized that more rotation would lead to too much loss on the borders of the images and as the scan are usually similarly in the spatial positioning of the layers it would mislead the model. The "reflect" fill-mode to obtain a more organic result to replace missing part of the images. We use an elastic function to compute the distortion, the function creates random displacement fields for height and width and smoothing the fields with a Gaussian. 
\begin{lstlisting}
...
We based our model on the U-net architecture due to it's flexibility to adapt to different kind of images. We used a combination of an implementation made by Dr. Ronchetti \cite{Ronchetti2019} and the youtuber DigitalSreeni \cite{DigitalSreeni}.

According to the original U-net model, the contracting path is a combination of convolution and pooling operations as illustrated can be demonstrated in Fig. \ref{fig:model}.
...
Our first convolutional layer has 64 filters as described in Lst. \ref{lst:conv2D-1}, we use a 3x3 kernel initialized by the keras initializer "he\_normal" which draws samples from a truncated normal distribution centered on 0 with $stddev = sqrt(2 / fan\_in)$ where fan\_in is the number of input units in the weight tensor. The layer uses the same padding to avoid the loss of data and the activation function is the Rectified Linear Unit (relu) which fires the neuron for every positive value resulting from the convolution. 

It's important to mention that in order to avoid overfitting we dropout 10\% of the output data that runs through the network and finally we apply a pooling operation with a 2*2 kernel size in order to propagate to the next neuronal layers the pixels with the most important features.

\begin{lstlisting}[caption={Implementation of the first level of the contracting path}, label={lst:conv2D-1}]
...
\begin{lstlisting} [caption={Implementation of the last level of the upsampling path}, label={lst:conv2D-9}]
...
In order to train the model, we implemented the fit method by providing the parameters shown in Lst. \ref{lst:fit-method}. Since we are using image augmentation and keras generator generates an unlimited number of images from our dataset we decided to show the algorithm only 100 images per epoch. Therefore, having 1000 epochs would mean that the model sees 100000 images generated from a the training set containing 604 images. Certain authors \cite{Shorten2019, mikolajcyk2018} would argue that providing the model with such a large quantity of generated images would bias our results towards our own data distribution and the model would not be able to generalize further. However, since our dataset stems from a very defined population we believe that by design the results of the model would already be biased towards the anatomic features of Asian eyes. Nevertheless, we implemented a cross validation method using 20\% of the original dataset whose results indicate that our training is not overfitting the data\footnote{See Sec. \ref{Results}}. Therefore, we do not believe that our model's ability to generalize the dataset is influenced by the length of the training or the number of images generated but the results are clearly biased towards a the Asian population and would be probably more successful for that kind of samples. The bias problem could only be solved by adding data from different populations so the model learns the characteristics of the human eye from all kinds of different humans. 

\begin{lstlisting}[caption={Model training parameters}, label={lst:fit-method}]
...
In order to evaluate the model we use different measures including Jaccard coefficient also called Intersection over Union (IoU). This measure is typically used to evaluate the similarity between two arbitrary shapes and is invariant to the scale of the problem considered because it encodes the shape properties of the objects being compared in the region property before computing a normalized measure that focuses on their surfaces \cite{IoU:2019}. The Jaccard coefficient can be described as follows \cite{Niwattanakul2013}:

\begin{equation}
    J(A,B) = \frac{|A\cap B|}{|A\cup B|}
\end{equation}

Jaccard's similarity coefficient between the two datasets A and B is the result of dividing between the number of common features of A and B, divided by the number of properties as shown above \cite{Niwattanakul2013}. 
This measure allows us to compare the similarity between our model prediction and the ground truth by returning the percentage of correctly classified pixels. 
This is also one of the reasons why the accuracy measure is not used as an indicator of how well the model works. Accuracy is measured by the number of correct predictions out of the total number of predictions, but when we have for example 97\% accuracy, it means that 3\% of the pixels are not well classified, with a 500x768 image this gives us 11'250 pixels that will be misclassified. The reason why we don't use the "accuracy" measure is because it loses precision as there is no separation between classes and it doesn't offer any information about the correct prediction portion of the image but rather on the overall set of predictions divided by the total of the possibilities. 
...
//TODO placement table
    \begin{table}[]
        ...
        \end{tabular}
    \end{table}
\\With this confusion matrix we can then compute two metrics that we use for our model evaluation: sensitivity and specificity. 
The sensitivity can be described, in our case, as the proportion of  classified pixels in a class among those that truly belong in this class or in other word the number of true positive over the sum of the true positive and false negative. 
The specificity is measuring the ability to correctly reject the pixels that do not belong in a class, or in other word the number of true negatives divided by the sum of the true negatives and false positives. Sensitivity measures  the proportion of actual positive cases that were correctly predicted, while specificity measure is used to determine the proportion of actual negative cases that were correctly predicted. This can help us to determine if our model is efficient in a general way (not per each class).
